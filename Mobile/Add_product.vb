...
Public Class Add_product
                        ...
                        If txt_brate.Text <> "" Then
                            If txt_sales_rate.Text <> "" Then
                                If flag = 0 Then
                                    ...
                                    Dim f As Integer = 0
                                    Dim dao As New Dao
                                    f = dao.validate("Select Product_Name from Product1 where Product_Name='" & txt_pname.Text & "'")
                                    dao.close_conn()
                                    If f = 1 Then
                                        MessageBox.Show("Unable to add Product it is already Created")
                                        txt_pname.Focus()
                                    Else
                                        dao.modifyData("insert into Product1 (Product_Name,Rate,GST,[CGST/SGST],Amount,Sales_Rate) values('" & txt_pname.Text & "','" & txt_rate.Text & "','" & txt_GST.Text & "','" & txt_cgst_sgst.Text & "','" & txt_brate.Text & "','" & txt_sales_rate.Text & "')")
                                ...
                                Else
                                    'update
                                    Dim dao As New Dao
                                    dao.modifyData("update Product1 set Product_Name='" & txt_pname.Text & "',Rate='" & txt_rate.Text & "',GST='" & txt_GST.Text & "',[CGST/SGST]='" & txt_cgst_sgst.Text & "',Amount='" & txt_brate.Text & "',Sales_Rate='" & txt_sales_rate.Text & "'where Product_id='" & txt_id.Text & "'")
    ...
    Public Sub loaddata()
        ...
        Dim d As New Dao
        ds = d.loaddata("select * from Product1")
        DataGridView1.DataSource = ds.Tables(0)
    End Sub

    ' Pre-load an existing product into the form for editing (used by Purchase Entry's product picker).
    Public Sub LoadProductForEdit(ByVal productName As String)
        Try
            Dim d As New Dao
            Dim ds As New Data.DataSet
            ds = d.loaddata("select * from Product1 where Product_Name='" & productName & "'")
            If ds.Tables(0).Rows.Count > 0 Then
                Dim r As DataRow = ds.Tables(0).Rows(0)
                txt_id.Text = r("Product_id").ToString()
                txt_pname.Text = r("Product_Name").ToString()
                txt_GST.Text = r("GST").ToString()
                txt_brate.Text = r("Amount").ToString()
                txt_sales_rate.Text = r("Sales_Rate").ToString()
                txt_cgst_sgst.Text = r("CGST/SGST").ToString()
                flag = 1
    ...
    Private Sub btn_del_Click(sender As Object, e As EventArgs) Handles btn_del.Click
        ...
        If c = 6 Then
            Dim d As New Dao
            d.modifyData("delete from Product1 where Product_id=" & txt_id.Text)
            loaddata()
            btn_clr_Click(sender, e)
        End If
        btn_del.Enabled = False
    ...
    Private Sub btn_del_MouseHover(sender As Object, e As EventArgs) Handles btn_del.MouseHover
        ...
        btn_del.BackColor = Color.Crimson
    ...
    Private Sub btn_del_MouseLeave(sender As Object, e As EventArgs) Handles btn_del.MouseLeave
        ...
        btn_del.ForeColor = Color.Crimson
    End Sub
